...
// write the geo file
void gmsh::geo::write(const std::string &filename)
{
...
// write points
void gmsh::geo::write_point(std::ofstream &file)
{
    file << std::scientific << std::setprecision(10);
    for (const auto &point : this->m_points)
    {
        file << "Point(" << point.tag() << ") = {";
        file << std::setw(20) << point.x() << ", ";
        file << std::setw(20) << point.y() << ", ";
        file << std::setw(20) << point.z() << "};" << std::endl;
    }
    file << std::defaultfloat;
}

// write lines
void gmsh::geo::write_line(std::ofstream &file)
{
    for (const auto &line : this->m_lines)
    {
        file << "Line(" << line.tag() << ") = {" << line.start_point_tag() << ", " << line.end_point_tag() << "};" << std::endl;
    }
}

// write surfaces
void gmsh::geo::write_surface(std::ofstream &file)
{
    for (const auto &surface : this->m_surfaces)
    {
        file << "Line Loop(" << surface.tag() << ") = {";
        for (size_t i = 0; i < surface.size(); i++)
        {
            file << surface.line_tag(i);
            if (i != surface.size() - 1)
            {
                file << ", ";
            }
        ...
        file << "};" << std::endl;
        file << "Plane Surface(" << surface.tag() << ") = {" << surface.tag() << "};" << std::endl;
    }
}

// write volumes
void gmsh::geo::write_volume(std::ofstream &file)
{
    for (const auto &volume : this->m_volumes)
    {
        file << "Surface Loop(" << volume.tag() << ") = {";
        for (size_t i = 0; i < volume.size(); i++)
        {
            file << volume.surface_tag(i);
            if (i != volume.size() - 1)
            {
                file << ", ";
            }
        ...
        file << "};" << std::endl;
        file << "Volume(" << volume.tag() << ") = {" << volume.tag() << "};" << std::endl;
    }
}

// write the entities
void gmsh::geo::write_entities(std::ofstream &file)
{
    write_point(file);
    write_line(file);
    write_surface(file);
