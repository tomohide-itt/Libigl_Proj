...
int main(int argc, char *argv[])
{
  ...
  Eigen::MatrixXd n_vecs;
  igl::per_face_normals(meshes[0].vertex_matrix(), meshes[0].face_matrix(), n_vecs);
  std::cout << "n_vecs: " << std::endl << n_vecs << std::endl;

  /*
  // Output the meshes
  igl::opengl::glfw::Viewer viewer;
  //viewer.data().set_mesh(meshes[0].vertex_matrix(), meshes[0].face_matrix());
  viewer.data().set_mesh(meshes[0].vertex_matrix(), OutF);
